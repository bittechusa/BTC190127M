package com.bit.dr;
	...
	public class SeleniumHelper 
	{
		...
		public void handleMultipleWindow()
		{
			...
			a.contextClick(element).sendKeys(Keys.ARROW_DOWN).sendKeys(Keys.ARROW_DOWN).sendKeys(Keys.ENTER).perform();
		...
		{
			elements=dr.findElements(By.tagName("iframe"));
			for(WebElement ele:elements)
			{
				if(ele.getAttribute("id").equals(value))
				{
		...
		public void getLinkText(String value)
		{
			elements=dr.findElements(By.tagName("a"));
			for(WebElement ele:elements)
			{
		...
		public void test1()
		{
			...
			dr.quit();//to close whole instance
			dr.get("");//go to url
			...
			//method from Webelement
			 element.clear();//to clear text box
			...
			js.executeScript("arguments[0].value='jghj'",dr.findElement(By.id("")));
			...
			//ExplicitWait
//			its for particular element . it is logical wait
//			webdriverwait and fluentwait is the example of explicit wait.
//			Using Expected conditions we can set different methods for logical wait like visibilityOf elementtobeclickable etc.
//			waitForElement(element).click();
		...
		{
			FluentWait wait=new FluentWait(dr);//polling concept
